...
public class FileBrowserActivity extends Activity {
	...
	public static final String toBeUploadedFileName = "com.protext.widget.FileBrowserActivity.toBeUploadedFileName";
	public static final String toBeUploadedFileURL = "com.protext.widget.FileBrowserActivity.toBeUploadedFileURL";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (thisInt.getAction().equalsIgnoreCase(INTENT_ACTION_SELECT_FILE)) {
			Log.e(LOGTAG, "SELECT ACTION - SELECT FILE");
		...
		Log.e(LOGTAG, path.getAbsolutePath());
		handler=new Handler();
	}
	Handler handler;
	...
	private void initializeButtons() {
		...
		upDirButton.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				Log.e(LOGTAG, "onclick for upDirButton");
				loadDirectoryUp();
				loadFileList();
		...
		if (currentAction == SELECT_DIRECTORY) {
			...
			selectFolderButton.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					Log.e(LOGTAG, "onclick for selectFolderButton");
...
//						+ "]");
	...
	private void initializeFileListView() {
		...
		lView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view,
				...
				Log.e(LOGTAG, "Clicked:" + chosenFile);
				if (sel.isDirectory()) {
					if (sel.canRead()) {
						// Adds chosen directory to list
						pathDirsList.add(chosenFile);
						path = new File(sel + "");
						Log.e(LOGTAG, "Just reloading the list");
						loadFileList();
						adapter.notifyDataSetChanged();
						updateCurrentDirectoryTextView();
						Log.e(LOGTAG, path.getAbsolutePath());
					...
					Log.e(LOGTAG, "item clicked");
					if (!directoryShownIsEmpty) {
						Log.e(LOGTAG, "File selected:" + chosenFile);
	...
	private void downloadFileInLocal(){
		new Thread(new Runnable() {@Override public void run() {
		java.io.File fileContent = new java.io.File(path, getIntent().getStringExtra(toBeUploadedFileName));
		showToast("Downloading file:\n"+fileContent.getName());
		HttpGet httpGet = new HttpGet(getIntent().getStringExtra(toBeUploadedFileURL));
		HttpClient httpclient = new DefaultHttpClient();
		org.apache.http.HttpResponse httpResponse;
		try {
			httpResponse = httpclient.execute(httpGet);
			org.apache.http.HttpEntity entity = httpResponse
					.getEntity();
	...
	private void loadFileList() {
		...
		if (path.exists() && path.canRead()) {
					...
					if (currentAction == SELECT_FILE) {
						...
						if (sel.isFile()) {// && filterFileExtension != null
							...
							if (mime != null && mime.toLowerCase().startsWith("text")) {
								...
								isAccepted=true;
							} else if (mime != null && mime.toLowerCase().startsWith("audio")) {
								isAccepted=true;
							}
			 			    return (showReadableFile && isAccepted);
			...
			for (int i = 0; i < fList.length; i++) {
				...
				if (sel.isDirectory()) {
					if (canRead) {
						drawableID = R.drawable.folder_close;
					} else {
						drawableID = R.drawable.folder_close;//folder_icon_light
					}
				}
				fileList.add(i, new Item(fList[i], drawableID, canRead));
			}// for (int i = 0; i < fList.length; i++) {
			if (fileList.size() == 0) {
				// Log.e(LOGTAG, "This directory is empty");
		...
		// Log.e(TAG, "loadFileList finished");
	}// private void loadFileList() {
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		if (newConfig.orientation == Configuration.ORIENTATION_LANDSCAPE) {
			Log.e(LOGTAG, "ORIENTATION_LANDSCAPE");
		} else if (newConfig.orientation == Configuration.ORIENTATION_PORTRAIT) {
			Log.e(LOGTAG, "ORIENTATION_PORTRAIT");
		}
	...
	public static String getMIME(String fName){
		//fName="abc."+FilenameUtils.getExtension(fName);
		String mime = URLConnection.guessContentTypeFromName(fName);
	    if(isNullOrEmpty(mime)){
			mime=MimeTypeMap.getSingleton().getMimeTypeFromExtension(MimeTypeMap.getFileExtensionFromUrl(fName));
		}if(isNullOrEmpty(mime)){
			try{mime = URLConnection.getFileNameMap().getContentTypeFor(fName);}catch(Exception e){}
		}
	    return mime;
	}
